...
<?php
require_once('C:\Users\wissem\Desktop\FormationPlateforme\password_compat\lib\password.php');
  		...
  		if ( ! (empty($_POST['uid'])|| trim($_POST['uid'])=='') && !(empty($_POST['mail'])|| trim($_POST['mail'])=='') && !(empty($_POST['pwd'])|| trim($_POST['pwd'])=='') && !(empty($_POST['pwd-repeat'])|| trim($_POST['pwd-repeat'])=='') ) {
  			...
  			if(!filter_var($_POST['mail'],FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/",$_POST['uid'])){
				...
				try{
					$db=new PDO('mysql:host=localhost;dbname=agencehana;charset=utf8','root','',array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
				}
				catch(Exception $err){
